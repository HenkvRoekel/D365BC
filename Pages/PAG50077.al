page 50077 "Purch. Ord. Preview as subform"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("gcuBlockingMgt.ImpPurchaseLineBlockDescr(Rec)"; gcuBlockingMgt.ImpPurchaseLineBlockDescr(Rec))
                {
                ...
                field("grecItem.""Purchases (Qty.)"""; grecItem."Purchases (Qty.)")
                {
                    ...
                    begin
                        ...
                        // grecItemLedgerEntry.SETCURRENTKEY("Item No.");
                        // grecItemLedgerEntry.SETRANGE("Item No.","No.");
                        // // grecItemLedgerEntry.SETRANGE("Posting Date",Voorraadinstellingen."Begin Date",Voorraadinstellingen."End Date"); BEJOWW5.0.001
                        // grecItemLedgerEntry.SETRANGE("Posting Date",grecBejoSetup."Begin Date",grecBejoSetup."End Date"); // BEJOWW5.0.001
                        // grecItemLedgerEntry.SETRANGE(grecItemLedgerEntry."Entry Type",grecItemLedgerEntry."Entry Type"::Purchase);
                        // FORM.RUNMODAL(FORM::"Item Ledger Entries",grecItemLedgerEntry);
                ...
                field("grecItem.""Qty. on Sales Order"""; grecItem."Qty. on Sales Order")
                {
                    ...
                    begin
                        ...
                        DrilldownField(grecItem."Qty. on Sales Order");
                    end;
                }
                field(gDisp; gDisp)
                {
    ...
    begin
        ...
        grecImportedPurchaseLines.Reset;
        grecImportedPurchaseLines.SetCurrentKey("Document Type", "Document No.", "No.");
        grecImportedPurchaseLines.SetRange("Document Type", "Document Type");
        grecImportedPurchaseLines.SetRange("Document No.", "Document No.");
        grecImportedPurchaseLines.SetRange("No.", "No.");
        grecImportedPurchaseLines.CalcSums("Qty. to Receive (Base)");
    ...
    begin
        ...
        if CopyStr(grecCompanyInformation."VAT Registration No.", 1, 2) = 'NL' then
            REPORT.Run(REPORT::"Delete Original Purchase Line", true, true, grecImportedPurchaseLines)
        else
