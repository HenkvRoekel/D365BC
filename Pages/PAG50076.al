page 50076 "MonthProgn Filtered FactBox"
{
    ...
    layout
    {
        area(content)
        {
            field(ggFilter_description; ggFilter_description)
            {
                StyleExpr = TRUE;
                ApplicationArea = All;
            }
            ...
            {
                Caption = 'Item';
                field(gItem_prognoses_filtered; gItem_prognoses_filtered)
                {
                    Caption = 'Prognoses';
                    DecimalPlaces = 0 : 4;
                    ApplicationArea = All;
                }
                field(gItem_sales_filtered; gItem_sales_filtered)
                {
                    Caption = 'Sales';
                    DecimalPlaces = 0 : 4;
                    ApplicationArea = All;
            ...
            {
                Caption = 'Unit';
                field(gUnit_prognoses_filtered; gUnit_prognoses_filtered)
                {
                    Caption = 'Prognoses';
                    DecimalPlaces = 0 : 4;
                    ApplicationArea = All;
                }
                field(gUnit_sales_filtered; gUnit_sales_filtered)
                {
                    Caption = 'Sales';
                    DecimalPlaces = 0 : 4;
                    ApplicationArea = All;
    ...
    var
        ...
        ggItem_Prognoses_filtered: Decimal;
        ggItem_sales_filtered: Decimal;
        ggUnit_prognoses_filtered: Decimal;
        ggUnit_sales_filtered: Decimal;
        emptyField: Text;
        emptyField2: Text;
    ...
    begin
        ...
        if _BudgetDim2Filter <> '' then
            SetRange("Unit of Measure", _BudgetDim2Filter)
        else
            Reset;
    ...
    begin
        ...
        if (GetFilter("Sales Person Filter") <> '') or (GetFilter("Customer Filter") <> '') then begin
            grecItemUnit.SetRange("Item No.", "Item No.");
            grecItemUnit.SetRange("Date Filter", gBeginDateLY, gEndDateLY);
            if GetFilter("Sales Person Filter") <> '' then
                grecItemUnit.SetRange("Sales Person Filter", GetFilter("Sales Person Filter"));
            if GetFilter("Customer Filter") <> '' then
                grecItemUnit.SetRange("Customer Filter", GetFilter("Customer Filter"));
            if grecItemUnit.Find('-') then
                repeat
                    grecItemUnit.CalcFields(Invoiced, Prognoses);
